...
class PCATrainer:
    ...
    def train(self, max_iters=5000, min_loss=0.01, log_interval=100, ue_progress=False):
        ...
        for i in range(max_iters):
            ...
            # Using MSE Loss to align with SVD initialization and preserve high frequencies
            loss = F.mse_loss(output, self.target)
            ...
            if current_loss <= min_loss:
                if ue_progress:
                    print(
                        f"[UE_PROGRESS] {i}/{max_iters} - Loss: {current_loss:.6f} (Target Reached)"
                    )
                break
    ...
    def _apply_constraints(self):
        ...
        with torch.no_grad():
            if hasattr(self.model, "inwhite") and hasattr(self.model, "inblack"):
                ...
                self.model.inwhite.copy_(
                    torch.clamp(
                        self.model.inwhite, min=self.model.inblack.item() + 0.001
                    )
                )
            if hasattr(self.model, "ingamma"):
                self.model.ingamma.copy_(
                    torch.clamp(self.model.ingamma, min=0.001, max=10.0)
                )
            if hasattr(self.model, "latent_maps"):
                self.model.latent_maps.copy_(
                    torch.clamp(self.model.latent_maps, min=0.0, max=1.0)
                )
